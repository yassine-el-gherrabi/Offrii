...
async fn get_wish_with_unknown_db_status_500() {
    ...
    // Corrupt the status directly in DB
    sqlx::query("UPDATE community_wishes SET status = 'garbage' WHERE id = $1")
        .bind(wish_id)
        .execute(&app.db)
        .await
        .unwrap();

    // get_wish: status parse returns None, not owner match → 404 (treated as invisible)
    let (status, _) = app
        .get_with_auth(&format!("/community/wishes/{wish_id}"), &token)
        .await;
    // Owner path: match status → None, then `_ if is_owner` → passes through.
    // WishStatus::parse returns None, but is_owner is true so it doesn't 404.
    // However the DTO construction may or may not fail depending on which fields
    // are used. The key behavior is: it doesn't panic.
    assert!(
...
#[tokio::test]
async fn close_wish_with_unknown_db_status_500() {
    let app = TestApp::new().await;
    let token = setup_aged_user(&app, "owner@test.com").await;
    let wish_id = create_open_wish(&app, &token).await;
    ...
    // close_wish calls WishStatus::parse with .ok_or_else → Internal error
    let (status, _) = app
        .post_with_auth(&format!("/community/wishes/{wish_id}/close"), &token)
        .await;
