...
mod tests {
    ...
    fn generate_access_token_produces_valid_jwt() {
        let token = test_keys()
            .generate_access_token(Uuid::new_v4(), 1, false)
            .unwrap();
    ...
    fn generate_refresh_token_produces_valid_jwt() {
        let token = test_keys()
            .generate_refresh_token(Uuid::new_v4(), 1, false)
            .unwrap();
        assert_eq!(token.matches('.').count(), 2);
    }
    ...
    fn valid_access_token_validates_with_correct_claims() {
        ...
        let token = keys.generate_access_token(user_id, 1, false).unwrap();
    ...
    fn valid_refresh_token_validates_with_correct_claims() {
        let keys = test_keys();
        let user_id = Uuid::new_v4();
        let token = keys.generate_refresh_token(user_id, 1, false).unwrap();
    ...
    fn jti_is_unique_across_tokens() {
        ...
        let t1 = keys.generate_access_token(user_id, 1, false).unwrap();
        let t2 = keys.generate_access_token(user_id, 1, false).unwrap();
    ...
    fn access_and_refresh_tokens_have_different_types() {
        ...
        let access_claims = keys
            .validate_access_token(&keys.generate_access_token(user_id, 1, false).unwrap())
            .unwrap();
        let refresh_claims = keys
            .validate_refresh_token(&keys.generate_refresh_token(user_id, 1, false).unwrap())
            .unwrap();
    ...
    fn token_version_round_trips_correctly() {
        ...
        let token_v3 = keys.generate_access_token(user_id, 3, false).unwrap();
        let claims = keys.validate_access_token(&token_v3).unwrap();
        assert_eq!(claims.token_version, 3);

        let token_v42 = keys.generate_refresh_token(user_id, 42, false).unwrap();
    ...
    #[test]
    fn is_admin_claim_persists_in_token() {
        let keys = test_keys();
        let user_id = Uuid::new_v4();

        // Admin token should have is_admin = true
        let admin_token = keys.generate_access_token(user_id, 1, true).unwrap();
        let claims = keys.validate_access_token(&admin_token).unwrap();
        assert!(claims.is_admin, "admin token should have is_admin = true");

        // Non-admin token should have is_admin = false
        let user_token = keys.generate_access_token(user_id, 1, false).unwrap();
        let claims = keys.validate_access_token(&user_token).unwrap();
        assert!(
            !claims.is_admin,
            "regular token should have is_admin = false"
        );
    }

    #[test]
    fn is_admin_defaults_false_for_legacy_tokens() {
        // Tokens without is_admin claim should default to false (backward compat)
        let keys = test_keys();
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
        ...
        let legacy = LegacyNoAdminClaims {
            sub: Uuid::new_v4().to_string(),
            exp: (now + 900) as usize,
            iat: now as usize,
            jti: Uuid::new_v4().to_string(),
        ...
        assert!(
            !claims.is_admin,
            "missing is_admin should default to false — prevents privilege escalation on old tokens"
        );
    }

    #[test]
    fn is_admin_in_refresh_token_too() {
        let keys = test_keys();
        let user_id = Uuid::new_v4();

        let refresh = keys.generate_refresh_token(user_id, 1, true).unwrap();
        let claims = keys.validate_refresh_token(&refresh).unwrap();
